...
class Generator(nn.Module):
    ...
    def forward(self, X, TE,norm_dis_matrix):
        X = torch.unsqueeze(X, -1)
        aggcn_output = self.AGGCN(X,norm_dis_matrix)
...
class AGGCN(nn.Module):
    ...
    def forward(self, source,norm_dis_matrix):
        ...
        output= self.encoder(source,norm_dis_matrix,init_state, self.node_embeddings, self.time_embeddings)
...
class AGRNN(nn.Module):
    ...
    def forward(self, x, norm_dis_matrix,init_state, node_embeddings, time_embeddings):
        ...
        for i in range(self.num_layers):
            ...
            for t in range(seq_length):
                state = self.dagrnn_cells[i](current_inputs[:, t, :, :], norm_dis_matrix, state, node_embeddings,time_embeddings[t])
...
class GRU(nn.Module):
    ...
    def forward(self, x, norm_dis_matrix,state, node_embeddings,  time_embeddings):
        state = state.to(x.device)
        input_and_state = torch.cat((x, state), dim=-1)
        z_r = torch.sigmoid(self.gate(input_and_state, norm_dis_matrix, node_embeddings,  time_embeddings))
        z, r = torch.split(z_r, self.hidden_dim, dim=-1)
        candidate = torch.cat((x, z * state), dim=-1)
        hc = torch.tanh(self.update(candidate, norm_dis_matrix, node_embeddings, time_embeddings))
        h = r * state + (1 - r) * hc
...
class GCN(nn.Module):
    ...
    def forward(self, x,norm_dis_matrix, node_embeddings,  time_embeddings):
        ...
        # supports = F.softmax(torch.mm(node_embeddings, node_embeddings.transpose(0, 1)) + norm_dis_matrix, dim=1)
